...
export class WidthHeightElementComponent implements OnInit {
    ...
    private initialDimensionValues = { height: 0, width: 0 };
    ...
    public ngOnInit(): void {
        ...
        this.initialDimensionValues = this.formElement.getRawValue();
    ...
    /**
     * An active element can only be updated after one of its visible dimensions
     * differs from the values that were loaded into the form. A 0 × 0 size is
     * not a valid update.
     */
    public shouldShowSaveButton(): boolean {
        const values = this.formElement.getRawValue();
        const bothDimensionsAreZero = Number(values.height) === 0 && Number(values.width) === 0;

        if (!this.element || !this.element.state.active) {
            return Number(values.height) > 0 && Number(values.width) > 0;
        }

        if (bothDimensionsAreZero) {
            return false;
        }

        const staticValues = this.element.element.definition.staticValues;

        return (
            (staticValues.renderingHeight !== 'none' && !this.areEqualDimensionValues(values.height, this.initialDimensionValues.height)) ||
            (staticValues.renderingWidth !== 'none' && !this.areEqualDimensionValues(values.width, this.initialDimensionValues.width))
        );
    }
    ...
    public saveInput(): void {
        ...
        // update button is only shown after the user changes a value afterwards.
        this.initialDimensionValues = this.formElement.getRawValue();
    ...
    private areEqualDimensionValues(value: unknown, initialValue: unknown): boolean {
