...
export class ImageCanvasComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private renderImagesSubscription: Subscription;
  ...
  public ngOnInit(): void {
    this.mediaUrl = `${environment.api.media}/`;

    // we need this debounceTime otherwise on resize canvas will be updated too many times and will be slow
    this.resizeSubject.pipe(
      debounceTime(100)
    ).subscribe(() => {
      // if canvas is ready then draw images
      if (this.ctx) {
        this.drawImages();
      }
    });
  ...
  public ngAfterViewInit(): void {
    ...
    this.renderImagesSubscription = this.renderImage$.pipe(
      debounceTime(100)
    ).subscribe((data) => {
      this.renderImages = data.images;

      if (this.firstImageWidth && this.firstImageHeight) {
  ...
  private get availableWidth(): number {
    return this.canvasContainerRef.nativeElement.clientWidth;
  }

  private get availableHeight(): number {
    return this.canvasContainerRef.nativeElement.clientHeight;
  }
  ...
  private clearCanvas(): void {
    if (this.canvasRef.nativeElement.width && this.canvasRef.nativeElement.height) {
  ...
  private adjustCanvasSize(): void {
    this.canvasRef.nativeElement.width = this.firstImageWidth;
    this.canvasRef.nativeElement.height = this.firstImageHeight;
    this.scale = 1;
  }
  ...
  public ngOnDestroy(): void {
    if (this.renderImagesSubscription) {
      this.renderImagesSubscription.unsubscribe();
    }
